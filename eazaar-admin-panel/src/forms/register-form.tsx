...
const RegisterForm = () => {
  ...
  const onSubmit = async (formData: {name:string;email:string;password:string}) => {
    const registrationResult = await registerAdmin({name: formData.name, email: formData.email, password: formData.password});
    if ("error" in registrationResult) {
      if ("data" in registrationResult.error) {
        const errorResponse = registrationResult.error.data as { message?: string };
        if (typeof errorResponse.message === "string") {
          return notifyError(errorResponse.message);
        }
      }
    } else {
